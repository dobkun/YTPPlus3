...
function Install-YTPPlusPlusPlus {
    # Write ASCII art
    Write-Host "$asciiart
    Welcome to the YTP+++ (Mono Edition) deploy script!
    Originally written by KiwifruitDev. Press Ctrl + Left Click to open links in your web browser.
    I wonder if I get a C&D followed by a subpoena by distributing FOSS that later became paid software.
...
" -ForegroundColor Yellow -BackgroundColor Black
        ...
        if ($continue -eq "y") {
            ...
            # Pull https://api.github.com/repos/dobkun/YTPPlus3/releases/latest to get the latest release and extract it to Desktop.
            Write-Host "Getting latest YTP+++ release..." -ForegroundColor Yellow
            $header = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
            $header.Add("User-Agent", "YTPPlusPlusPlus")
            $ytp = Invoke-WebRequest -Uri "https://api.github.com/repos/dobkun/YTPPlus3/releases/latest" -Headers $header -OutFile "$env:TEMP\ytp.json"
    ...
    Want to contribute? Head to https://github.com/dobkun/YTPPlus3 and make a pull request!
